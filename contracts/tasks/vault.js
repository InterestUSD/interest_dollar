...
async function harvest(taskArguments, hre) {
  const { isMainnet, isAlfajores, isFork } = require("../test/helpers");
  const { executeProposal } = require("../utils/deploy");
  const { proposeArgs } = require("../utils/governor");

  if (isMainnet || isAlfajores) {
    throw new Error("The harvest task can not be used on mainnet or alfajores");
  }
...
async function reallocate(taskArguments, hre) {
  const { isFork, isMainnet, isAlfajores } = require("../test/helpers");
  const { formatUnits } = hre.ethers.utils;

  if (isMainnet || isAlfajores) {
    throw new Error("reallocate task can not be used on Mainnet or Alfajores");
  }
